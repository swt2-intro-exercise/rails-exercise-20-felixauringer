...
describe 'Submitting the new author form', type: :feature do
  let(:create_author_action) { click_button('commit') }
  ...
  describe 'with valid attributes' do
    ...
    it 'adds a new author to the database' do
      expect { create_author_action }.to change(Author, :count).by(1)
  ...
  describe 'with invalid attributes' do
    ...
    it 'does not add the author to the database' do
      expect { create_author_action }.not_to change(Author, :count)
    end
